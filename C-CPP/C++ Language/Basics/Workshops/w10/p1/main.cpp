...
int main()
{
    ...
    for (ii = 0; ii < 3; ii++) {
        ...
        index = find<Card,int,const char*>(cCol1,cCol1.size(),valuesToSearch[ii],suits[ii]); //****TODO 2:
        if (index >= 0) {
            //add the found object at the corresponding index to cCol2. (Review the Collection code for the correct operator) 
            loadCollection<Card>(cCol2,cCol1[index]);   //****TODO 4:
        }
    }

    //Search for all "Hearts" cards
    for (ii = 0; ii < 7; ii++) {
        if (find<Card,const char*>(cCol1,ii,"Hearts")) { //****TODO 1:
    ...
    for (ii = 0; ii < 4; ii++) {
        //call the 4 argument find function with values from the 2 above arrays
        index = find<Book,int,const char*>(bCol,bCol.size(),upcToSearch[ii],titlesToSearch[ii]); //****TODO 2:
        if (index >= 0) {
            //add the found object at the corresponding index to cCol2. (Review the Collection code for the correct operator) 
            loadCollection<Book>(bCol2,bCol[index]);     //****TODO 4:
        }
        else {
    ...
    for (ii = 0; ii < 6; ii++) {
        if (find<Pet,const char*>(pCol,ii,"Dog")) { //****TODO 1:
    ...
    Collection<int> iColA;
    for(int i=0;i<6;i++)
    {
        loadCollection<int>(iColA,a[i]);
    }
    ...
    for (ii = 0; ii < 6; ii++) {
        if (find<int,int>(iColA,ii,90)) { //****TODO 1:
            loadCollection(iCol,iColA[ii]); //****TODO 4:
        }
    }

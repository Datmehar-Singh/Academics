...
CustomerOrder::CustomerOrder(){
	...
	CustomerOrder::CustomerOrder(const std::string& wholeLine){
		...
		bool more=true;
		m_lstItem=nullptr;
		m_name=u.extractToken(wholeLine,next,more);
		m_product=u.extractToken(wholeLine,next,more);
Item** arr=new Item*[10];
int i=0;
		while(more){

arr[i]=new Item{u.extractToken(wholeLine,next,more)};
i++;
}
m_cntItem=i;
...
c.m_lstItem=nullptr;
			...
			void CustomerOrder::fillItem(Station& station, std::ostream& os){
				...
				if(flag==false)
				{
					...
					m_lstItem[m_cntItem-1]=new Item{station.getItemName()};
					delete[] temp;
				}
				return;
			}
